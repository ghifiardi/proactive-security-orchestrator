...
### 2. **IMPLEMENTATION_GUIDE.md**
   - **What**: Step-by-step instructions to use Cursor for Research → Plan → Implement
   - **Length**: ~400 lines
   - **Purpose**: Walk through all three phases, from setup to complete
   - **When to read**: Before opening Cursor (10 min)
   - **Action**: Follow Part 1 (setup) → Part 5 (implementation)
...
### `.agents/` – Workflow Artifacts (Output of Each Phase)
...
| **plan.md** | Phase B (Plan) | Implementation plan: Langkah 1–12, per-file changes, acceptance criteria | After Phase B, before Phase C |
| **progress.md** | Phase C (Implement) | Execution log: Langkah N--Ringkasan (≤20 lines each, step-by-step) | During Phase C, after each Langkah |
...
**How to read**: Open in your editor → scroll through → understand the context

**Example**: `cat .agents/research.md | less`

---

### `prompts/` – LLM Prompt Templates (Reusable, Phase-Specific)
...
|------|-------|---------|--------|-----------|
| **parent_system_prompt.md** | A, B, C | System prompt for Cursor Composer; defines orchestrator rules & child agents | ~400 lines | **Set as SYSTEM prompt once** → keep for all 3 phases |
...
4. Run → generates artifacts

**Example flow**:
```
Phase A: System + research_prompt.md → generates research.md
Phase B: System + plan_prompt.md → generates plan.md
Phase C: System + implement_prompt.md (Langkah 1) → diff + progress update
Phase C: System + implement_prompt.md (Langkah 2) → diff + progress update
... repeat for all 12 Langkah
```

---

### `contracts/` – JSON Schemas (Data Validation)
...
### `.cursorrules` – Workflow Guardrails
...
**Template sections**:
- Context (links to research.md §, plan.md § Langkah N)
...
### `src/` – Core Implementation (Generated During Phase C)

These files are **created/modified during implementation**. Start empty; populated by Langkah 1–12.
...
### `tests/` – Test Suite (Generated During Phase C)
...
| **IMPLEMENTATION_GUIDE.md** | Step-by-step walkthrough (Part 1–6) |
| **INDEX.md** | This file |
...
### Phase C: IMPLEMENT (12 Langkah × 45 min = 9 hours)
...
2. Follow Part 1 (setup) → Part 2 (Cursor prep) → Part 3 (Phase A)
...
### Paste into Cursor (Research → Plan → Implement)
...
Phase C done? Check: .agents/progress.md has 12 entries, all ✅
```
...
- [ ] Repeat for Langkah 2–12

### Post-Implementation
- [ ] All tests pass (≥80% coverage)
- [ ] .agents/progress.md shows "Complete ✅"
...
2. **INDEX.md** (this file) — How is it organized?
3. **IMPLEMENTATION_GUIDE.md** — How do I use it?
4. **prompts/parent_system_prompt.md** — What are the rules?
5. **.cursorrules** — What does Cursor enforce?
6. **.agents/research.md** (after Phase A) — What did we discover?
7. **.agents/plan.md** (after Phase B) — What are we building?
8. **.agents/progress.md** (during Phase C) — What did we build?
...
**Workflow**: Research → Plan (per-file) → Implement with Parent/Child orchestration
**Test coverage**: ≥80% (enforced by pytest)
